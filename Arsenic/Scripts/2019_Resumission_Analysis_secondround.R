...
biopruned_pr <- biopruned %>%
  ...
  dplyr::filter(trait %in% traits_for_pca_followup)
...
subtract_dr_to_pc <- biopruned_pr %>%
  ...
  dplyr::select(strain:u_strain,traits_for_pca_followup)
...
dr_plt_df <- dr_traits%>%
  ...
  dplyr::filter(Trait %in% c(traits_for_pca_followup, "PC1")) %>%
...
dr_plots <- cowplot::plot_grid(dr_plots[[2]]+theme(axis.title.x = element_blank(),
                               ...
                               dr_plots[[1]]+theme(axis.title.x = element_blank(),
                                                   axis.text.x = element_blank(),
                                                   legend.position = "none"),
                               dr_plots[[3]]+theme(axis.title.x = element_blank(),
                                                   axis.text.x = element_blank(),
                                                   legend.position = "none"),
                               dr_plots[[4]]+theme(axis.title.x = element_blank(),
                                                   axis.text.x = element_blank(),
                                                   legend.position = "none"),
                               dr_plots[[5]]+theme(legend.position = "none"),
                               labels = "AUTO", 
                               ncol = 1, 
                               align = 'v',
                               vjust = 1, 
...
plt_df <- h2_df%>%
  ...
  dplyr::mutate(tidy_cond = factor(condition, 
...
ggplot(plt_df)+
  ...
  ggrepel::geom_label_repel(aes(label = trait),
                   data = plt_df,
...
arsenic_linkage_processed <- RIAILs2regressed %>%
  ...
  dplyr::select(Condition = condition, 
                ...
                Trait = trait, 
                Value = phenotype) %>%
  dplyr::filter(Trait %in% traits_for_pca_loose)
...
pxg_linkage_length <- pxgplot_edit(arsenic_cross, dplyr::filter(lods,  trait == ".mean.TOF"))+ 
  scale_x_discrete(breaks=c("N2", "CB4856"),labels=c("N2", "CB4856"))+
...
pxg_linkage_opt <- pxgplot_edit(arsenic_cross, dplyr::filter(lods,  trait == ".mean.norm.EXT"))+ 
  scale_x_discrete(breaks=c("N2", "CB4856"), labels=c("N2", "CB4856"))+
  labs(y = paste0("Optical Density"))

pxg_linkage_flu <- pxgplot_edit(arsenic_cross, dplyr::filter(lods,  trait == ".mean.norm.yellow"))+ 
  scale_x_discrete(breaks=c("N2", "CB4856"), labels=c("N2", "CB4856"))+
  labs(y = paste0("Fluoescence"))
...
plot_grid(pxg_linkage_PC,
          ...
          pxg_linkage_opt, 
          pxg_linkage_flu,
          labels = "AUTO", ncol = 1, align = 'v', label_size = 18)
...
lods %>%
  # dplyr::filter(trait %in% c(".mean.TOF",".norm.n", ".PC1",)) %>%
  maxlodplot_edit3() +
  arsenic.theme +
...
broad_estimates <- ggplot(riail_h2_df)+
  ...
  ggrepel::geom_label_repel(aes(x = H2_realized, y = H2_expectation, label = trait),
                            ...
                            data = riail_h2_df %>%
                              dplyr::mutate(tidy_ve = round(QTL_II_VE/H2_expectation, digits = 3),
                                            tidy_trait = case_when(
                                              ...
                                              trait == "mean.TOF" ~ glue::glue("Animal Length"),
                                              TRUE ~ "NA"
                                            ))) +
...
broad_narrow_realized <- ggplot(riail_h2_df)+
  ...
  theme_classic(16) +
  geom_abline(slope=1,intercept = 0, linetype = 2, alpha = 0.5)+
  ggrepel::geom_label_repel(aes(x = H2_realized, y = h2_realized,label = trait),
                            min.segment.length = 0.01,
                            nudge_y=0.2,
                            segment.alpha = 0.5,
                            data = riail_h2_df %>%
...
broad_narrow_expected <- ggplot(riail_h2_df)+
  ...
  scale_color_manual(values = c("red","black")) +
  theme_classic(16) +
  ggrepel::geom_label_repel(aes(x = H2_expectation, y = h2_expectation,label = trait),
                            min.segment.length = 0.01,
                            nudge_y=0.2,
                            segment.alpha = 0.5,
                            data = riail_h2_df %>%
...
riail_p_cor_plot <- riail_lm %>%
  ...
  #                                       "x + ", unique(intercept), 
  #                                       "\n R^2 = ", unique(adj.r.squared), "; p = ", unique(pvalue))),
  #           data = dplyr::filter(riail_lm, CorTrait == "mean.TOF")) +
...
# 3.641
# N2 - left NIL
# 2.839 - 78% recapitulation
# N2 - right NIL
# 3.46 - 95% recapitulation

# Brood size and animal length NILs

nil_pheno%>%
  dplyr::filter(Trait %in% c("norm.n", "mean.TOF","mean.norm.EXT"),
                Strain%in%c("N2","CB4856","ECA414","ECA434"))%>%
  dplyr::mutate(strain1 = factor(Strain, levels = c("N2","CB4856","ECA414","ECA434","ECA581",
                                                    "ECA582","ECA589","ECA590","ECA591"),
                                 labels =c("N2",
                                           ...
                                           "CB4856(S78C)\nA","CB4856(S78C)\nB",
                                           "CB4856(S78C)\nC")),
                tidy_trait = factor(Trait, levels = c("norm.n", "mean.TOF","mean.norm.EXT"), 
                                    labels = c("Brood Size", "Animal Length", "Optical Density")))%>%
  ggplot(.) +
  aes(x = factor(strain1), 
      y = Value, 
      fill=Strain) +
  ...
  scale_fill_manual(values = c("N2" = "orange","CB4856" = "blue",
                               ...
                               "ECA591" = "gray50"))+
  theme_bw()+
  facet_grid(tidy_trait~., scales = "free")+
  arsenic.theme +
  labs( y = paste0("PC 1"))+
  theme(legend.position="none",
        strip.background = element_blank(),
...
nil_plot <- nil_lm %>%
  dplyr::filter(CorTrait %in%  c("mean.TOF","norm.n"))%>%
  ggplot()+
  aes(x = (value), y = PC1, color = CorTrait)+
  geom_hline(yintercept = 0, color = "gray50", alpha = 0.5) +
  geom_vline(xintercept = 0, color = "gray50", alpha = 0.5) +
  geom_point(aes( )) + 
  geom_smooth(method='glm',formula=y~x, se = F) +
  scale_color_manual(values = c("hotpink3", "cadetblue3", "red"), 
                     breaks = c("mean.TOF","norm.n"),
  ...
  geom_text(aes(30, 2, 
                label = paste("Adj R2 = ", adj.r.squared, "\n",
                              "Intercept =",intercept, "\n",
                              "Slope =", slope, "\n",
                              "P =", pvalue)), data = dplyr::filter(nil_lm, CorTrait == "mean.TOF"))+
  theme(panel.grid = element_blank())
nil_plot

nil_ext_plot <- nil_lm %>%
  dplyr::filter(CorTrait %in%  c("mean.norm.EXT"))%>%
  ggplot()+
  aes(x = (value), y = PC1, color = CorTrait)+
  geom_hline(yintercept = 0, color = "gray50", alpha = 0.5) +
  geom_vline(xintercept = 0, color = "gray50", alpha = 0.5) +
  ...
  scale_color_manual(values = c("red", "orange"), 
                     breaks = c("mean.norm.EXT"),
                     labels = c("Optical\nDensity"))  +
  theme_bw(18) +
  labs(x = "Phenotype Value", color = "Trait", y = "PC 1") +
  arsenic.theme +
  geom_text(aes(-0.25, -2, 
                label = paste("Adj R2 = ", adj.r.squared, "\n",
                              "Intercept =",intercept, "\n",
                              "Slope =", slope, "\n",
                              "P =", pvalue)), data = dplyr::filter(nil_lm, CorTrait == "mean.norm.EXT")) +
  theme(panel.grid = element_blank())

nil_fluor_plot <- nil_lm %>%
  dplyr::filter(CorTrait %in%  c("mean.norm.yellow"))%>%
  ggplot()+
  aes(x = (value), y = PC1, color = CorTrait)+
  geom_hline(yintercept = 0, color = "gray50", alpha = 0.5) +
  geom_vline(xintercept = 0, color = "gray50", alpha = 0.5) +
  ...
  scale_color_manual(values = c( "orange"), 
                     breaks = c("mean.norm.yellow"),
                     labels = c("Fluorescence"))  +
  theme_bw(18) +
  labs(x = "Phenotype Value", color = "Trait", y = "PC 1") +
  arsenic.theme +
  geom_text(aes(-0.02, -2, 
                label = paste("Adj R2 = ", adj.r.squared, "\n",
                              "Intercept =",intercept, "\n",
                              "Slope =", slope, "\n",
                              "P =", pvalue)), data = dplyr::filter(nil_lm, CorTrait == "mean.norm.yellow")) +
  theme(panel.grid = element_blank())
...
ggsave(glue::glue("{resub.plots}FS11_NIL_PC_length_brood_correlation.pdf"), 
       height = 6, 
       width = 12)
...
# GWAS PC analysis
arsenic_pca_ls <- run_pca(arsenic_pca, format = "w", drop_method = "s", n_pc = NA)
...
# need to combine two different phenotype sets that got input to PCA

file_three_traits <- grep("processed_mapping",list.files(glue::glue("{resub.data}Arsenic_GWA_BROOD_EXT_TOF/Mappings/Data")), value = T)

three_input_phenos <- list()
for(pr_maps in 1:length(file_three_traits)){
  three_input_phenos[[pr_maps]] <- readr::read_tsv(glue::glue("{resub.data}Arsenic_GWA_BROOD_EXT_TOF/Mappings/Data/{file_three_traits[pr_maps]}"))%>%
    na.omit() %>%
    dplyr::distinct(trait, strain, value) %>%
    dplyr::mutate(trait_input = "EXT_BROOD_TOF")
}
three_input_phenos <- dplyr::bind_rows(three_input_phenos)

file_four_traits <- grep("processed_mapping",list.files(glue::glue("{resub.data}Arsenic_GWA_BROOD_FLUOR_EXT_TOF/Mappings/Data")), value = T)
four_input_phenos <- list()
for(pr_maps in 1:length(file_three_traits)){
  four_input_phenos[[pr_maps]] <- readr::read_tsv(glue::glue("{resub.data}Arsenic_GWA_BROOD_FLUOR_EXT_TOF/Mappings/Data/{file_four_traits[pr_maps]}"))%>%
    na.omit() %>%
    dplyr::distinct(trait, strain, value) %>%
    dplyr::mutate(trait_input = "EXT_BROOD_TOF_FLUOR")
}

four_input_phenos <- dplyr::bind_rows(four_input_phenos)

both_sets <- dplyr::bind_rows(three_input_phenos,four_input_phenos) 

write.table(both_sets, 
            file = glue::glue("{resub.data}Supplemental_Data17_GWAS_ALL_TRAITS.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
...
new_pheno <- readr::read_tsv(file = glue::glue("{resub.data}Supplemental_Data17_GWAS_ALL_TRAITS.tsv"))%>%
  as.data.frame() %>%
  # tidyr::gather(trait,value, -strain,-trait_input) %>%
  dplyr::mutate(pheno_set = paste(trait, trait_input, sep = "_"))

narrow_h2 <- list()
for(i in 1:length(unique(new_pheno$pheno_set))){
  
  temp_pheno <- new_pheno %>%
    dplyr::filter(pheno_set == unique(new_pheno$pheno_set)[i]) 
...
pt_df <- h2_df %>%
  ...
  dplyr::mutate(cpoint = ifelse(trait == "PC1", 1,
                                ifelse(trait == "norm.n",3,
                                       ifelse(trait == "mean.TOF", 2, 
                                              ifelse(trait == "mean.norm.EXT", 4, 5)))),
                tidy_cond = factor(condition, 
                                   levels = c( "arsenic1", "arsenic2", "arsenic3","arsenic4"),
                                   labels = c( "250 µM", "500 µM", "1000 µM","2000 µM")))
...
ggplot(pt_df)+
  ...
  geom_point(color = "gray50", 
             data = dplyr::filter(pt_df, !trait %in% c("PC1","norm.n","mean.TOF", "mean.norm.EXT"),
                                  condition == "arsenic3")) +
  geom_point(aes(color = factor(cpoint, labels=c("PC1","Animal Length","Brood Size", "Optical Density")),
                 size = factor(cpoint, labels=c("PC1","Animal Length","Brood Size", "Optical Density")),
                 fill = factor(cpoint, labels=c("PC1","Animal Length","Brood Size", "Optical Density")),
                 shape = factor(cpoint, labels=c("PC1","Animal Length","Brood Size", "Optical Density"))),
             data = dplyr::filter(pt_df, trait %in% c("PC1","norm.n","mean.TOF", "mean.norm.EXT"),
                                  condition == "arsenic3"))+
  scale_shape_manual(values=c(21,21,21,21,1))+
  scale_color_manual(values=c("black","black","black","black","gray50"))+
  scale_fill_manual(values=c("red","hotpink3","cadetblue3","orange","gray50"))+
  scale_size_manual(values=c(3,3,3,3,1))+
...
# 
# Fit: aov(formula = stat_df$phenotype ~ stat_df$strain)
# 
# $`stat_df$strain`
# diff       lwr       upr     p adj
# ECA581-CB4856 0.9930524 0.6371558 1.3489491 0.0000000
# ECA590-CB4856 3.3784580 3.0247118 3.7322042 0.0000000
# N2-CB4856     3.9141512 3.5604050 4.2678974 0.0000000
# ECA590-ECA581 2.3854056 2.0295089 2.7413022 0.0000000
# N2-ECA581     2.9210988 2.5652021 3.2769954 0.0000000
# N2-ECA590     0.5356932 0.1819470 0.8894394 0.0007155

# brood and size
swap_pheno%>%
  dplyr::filter(Trait %in% c("mean.TOF", "norm.n", "mean.norm.EXT"),
                Strain != "ECA591", 
                Strain != "ECA414", 
                Strain != "ECA434", 
                Strain != "ECA582", 
  ...
  dplyr::mutate(strain1 = factor(Strain, 
                                 ...
                                 labels =c("N2\nDBT-1(C78)", 
                                           ...
                                           "CB4856\nDBT-1(C78)" ),
                                 ordered = T),
                tidy_trait = ifelse(Trait == "norm.n", "Brood Size", 
                                    ifelse(Trait =="mean.TOF", "Animal Length", "Optical Density")))%>%
  ggplot(.) +
  aes(x = factor(strain1), 
      y = Value, 
      fill=Strain) +
...
run_TukeyHSD(sig_swap,trt = "mean.TOF")
# Tukey multiple comparisons of means
# 95% family-wise confidence level
# 
# Fit: aov(formula = stat_df$phenotype ~ stat_df$strain)
...
# $`stat_df$strain`
# diff       lwr        upr     p adj
# ECA581-CB4856  -7.684092 -13.24145  -2.126735 0.0024544
# ECA590-CB4856 -37.169813 -42.69359 -31.646035 0.0000000
# N2-CB4856     -56.646866 -62.17064 -51.123089 0.0000000
# ECA590-ECA581 -29.485721 -35.04308 -23.928364 0.0000000
# N2-ECA581     -48.962774 -54.52013 -43.405417 0.0000000
# N2-ECA590     -19.477053 -25.00083 -13.953276 0.0000000
...
run_TukeyHSD(sig_swap,trt = "mean.norm.EXT")
# Tukey multiple comparisons of means
# 95% family-wise confidence level
# 
# Fit: aov(formula = stat_df$phenotype ~ stat_df$strain)
...
# $`stat_df$strain`
# diff        lwr         upr p adj
# ECA581-CB4856 -0.1321696 -0.1734066 -0.09093259     0
# ECA590-CB4856 -0.3465065 -0.3874943 -0.30551865     0
# N2-CB4856     -0.4968199 -0.5378077 -0.45583203     0
# ECA590-ECA581 -0.2143369 -0.2555739 -0.17309988     0
# N2-ECA581     -0.3646503 -0.4058873 -0.32341326     0
# N2-ECA590     -0.1503134 -0.1913012 -0.10932554     0
...
biopruned <- bioprune(summedraw)%>%
  dplyr::select(date:col,traits_for_pca_followup) %>%
...
rescue_pc_df <- data.frame(Strain = rescue_processed$Strain,
                           ...
                           PC1 = rescue_arsenic_pca_analysis$x[,1:pc_ve_90]) %>%
...
rescue_rotation_df <- data.frame(trait = row.names(rescue_arsenic_pca_analysis$rotation),
                                 PC1 = rescue_arsenic_pca_analysis$rotation[,1:pc_ve_90])
...
rescue_lm %>%
  ...
  geom_text(aes(200, -3, 
                label = paste("Adj R2 = ", adj.r.squared, "\n",
                              ...
                              "Slope =", slope, "\n",
                              "P =", pvalue)), 
            data = dplyr::filter(rescue_lm, CorTrait == "mean.TOF"))+
  theme(panel.grid.minor = element_blank())
...
# c15iso rescues 0.5813953 of the phenotypic difference, between n2 and n2 swap 
(0.885 - 1.51)/(0.885 - 1.96)
