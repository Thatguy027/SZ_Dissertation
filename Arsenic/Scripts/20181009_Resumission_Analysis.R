...
write.table(dr_trait_correlations,
            file = glue::glue("{resub.data}Supplemental_Data2_DR-TRAIT_COR.tsv"),
            sep = "\t",
            quote = F,
            col.names = T,
            row.names = F)
...
write.table(dr_rotation_df,
            file = glue::glue("{resub.data}Supplemental_Data3_DR-PCA-LOADINGS_Per_Condition.tsv"),
            sep = "\t",
            quote = F,
            col.names = T,
            row.names = F)
...
write.table(dr_complete_pc_df,
            file = glue::glue("{resub.data}Supplemental_Data4_DR-PCA-TRAITS_Complete_Experiment.tsv"),
            sep = "\t",
            quote = F,
            col.names = T,
            row.names = F)
...
write.table(dr_complete_rotation_df,
            file = glue::glue("{resub.data}Supplemental_Data5_DR-PCA-LOADINGS_Complete_Experiment.tsv"),
            sep = "\t",
            quote = F,
            col.names = T,
            row.names = F)
...
write.table(effects_df,
            file = glue::glue("{resub.data}Supplemental_Data6_DR-PCA_Effect-Sizes.tsv"),
            sep = "\t",
            quote = F,
            col.names = T,
            row.names = F)
...
write.table(broad_h2_df,
            file = glue::glue("{resub.data}Supplemental_Data7_DR-Heritability_estimates.tsv"),
            sep = "\t",
            quote = F,
            col.names = T,
            row.names = F)
...
for(cond in 1:length(unique(dr_traits$Condition))){
  ...
  ggsave(glue::glue("{resub.plots}FS1{tidy_subplot}_Trait-correlation_loadings.pdf"), 
         height = 12, 
         width = 20)
...
ggsave(glue::glue("{resub.plots}FS2_DR.pdf"), 
       height = 10, 
       width = 10)
...
write.table(gwas_trait_correlations, 
            file = glue::glue("{resub.data}Supplemental_Data20_GWAS_Trait_Correlations.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
            row.names = F )
...
new_pheno <- readr::read_tsv(file = glue::glue("{resub.data}Supplemental_Data15_GWAS_ALL_TRAITS.tsv"))%>%
  as.data.frame() %>%
...
write.table(gwas_h2, 
            file = glue::glue("{resub.data}Supplemental_Data20_GWAS_Genomic_H2_Estimates.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
            row.names = F )
...
# ggplot(pt_df)+
#   aes(x = DR_H2, y = Broad_H2_add_model)+
#   geom_point(color = "gray50", 
...
#   scale_size_manual(values=c(3,3,3,1))+
#   facet_grid(~tidy_cond) +
#   labs(x = "Dose Response - H2", y = "GWAS - H2 Additive Model",
#        color="Trait",
#        size = "Trait",
#        fill="Trait",
#        shape="Trait")+
...
#         panel.grid.minor = element_blank())
# 
# ggsave(glue::glue("{resub.plots}FS10_GWAS-H2_v_DR-h2.pdf"), 
#        height = 8, 
#        width = 16)

# ggplot(pt_df)+
#   aes(x = DR_H2, y = Broad_H2_add_epp_model)+
#   geom_point(color = "gray50", 
#              data = dplyr::filter(pt_df, !trait %in% c("PC1","norm.n","median.TOF"))) +
#   geom_point(aes(color = factor(cpoint, labels=c("PC1","Animal Length","Brood Size")),
#                  size = factor(cpoint, labels=c("PC1","Animal Length","Brood Size")),
...
#   scale_size_manual(values=c(3,3,3,1))+
#   facet_grid(~tidy_cond) +
#   labs(x = "Dose Response - H2", y = "GWAS - H2 Add-Epistatic Model",
#        color="Trait",
#        size = "Trait",
#        fill="Trait",
#        shape="Trait")+
...
#         panel.grid.minor = element_blank())
# 
# ggsave(glue::glue("{resub.plots}FS11_GWAS-H2epp_v_DR-h2.pdf"), 
#        height = 8, 
#        width = 16)
...
########################################################################################################################
# GWA MAPPING - H2 overlaid with mapping to dbt-1 - START
########################################################################################################################

gwas_h2 <- data.table::fread(glue::glue("{resub.data}Supplemental_Data20_GWAS_Genomic_H2_Estimates.tsv")) 
independent_tests <- data.table::fread(glue::glue("{resub.data}Supplemental_Data23_total_independent_tests.txt"))

qtl_summary <- data.table::fread(glue::glue("{resub.data}QTL_peaks.tsv")) %>%
  dplyr::mutate(dbt_peak = ifelse(V3 < 7931252 & 7931252 < V5, TRUE, FALSE)) %>%
  dplyr::select(trait = V1, dbt_peak) %>%
  dplyr::left_join(gwas_h2, .) %>%
  dplyr::arrange(desc(dbt_peak))%>%
  dplyr::distinct(trait, .keep_all=T)

qtl_summary[is.na(qtl_summary)] <- F

non_bf <- dplyr::filter(qtl_summary, dbt_peak == F)
non_bf_dbt <- data.frame()
for(trt in non_bf$trait){
  temp <- data.table::fread(glue::glue("~/Dropbox/AndersenLab/LabFolders/Stefan/Manuscripts/Arsenic/Resubmission/GWAS_ANALYSIS/Analysis_Results_Arsenic_Traits-20181008/Mappings/Data/{trt}_processed_mapping.tsv")) %>%
    dplyr::filter(POS == 7931252) %>%
    dplyr::select(trait,log10p)
  
  if(nrow(non_bf_dbt) ==0){
    non_bf_dbt <- temp
  } else {
    non_bf_dbt <- dplyr::bind_rows(non_bf_dbt,temp)
  }
  
}

eigen_bf_comb <- non_bf_dbt %>%
  dplyr::mutate(eigenthresh = ifelse(log10p>-log10(0.05/independent_tests$V1[1]), TRUE, FALSE))%>%
  dplyr::select(-log10p) %>%
  dplyr::left_join(qtl_summary, .) %>%
  dplyr::mutate(sig_thresh = ifelse(dbt_peak==T, "Bonferroni",
                                    ifelse(eigenthresh==T, "Eigen", "Not Significant")))

ggplot(eigen_bf_comb)+
  aes(x = Broad_H2_add_epp_model, y = narrow, fill = sig_thresh)+
  geom_point(size = 3, shape = 21)+
  scale_fill_manual(values = c("cadetblue3", "hotpink3", "gray50"))+
  theme_classic(18)

ggplot(eigen_bf_comb)+
  aes(x = Broad_H2_add_epp_model, fill = sig_thresh)+
  geom_density(alpha = 0.5)+
  scale_fill_manual(values = c("cadetblue3", "hotpink3", "gray50"))+
  theme_classic(18) +
  labs(x=expression(italic(H)^2), y = "Density",
       fill = "Significance\nThreshold")

ggsave(glue::glue("{resub.plots}FS15_chr2_peak_by_trait.pdf"), 
       height = 6, 
       width = 10)

write.table(eigen_bf_comb %>% dplyr::select(trait, sig_thresh), 
            file = glue::glue("{resub.data}Supplemental_Data21_Traits_with_dbt1_peak.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
            row.names = F )
...
########################################################################################################################
# GWA MAPPING - H2 overlaid with mapping to dbt-1 - END
########################################################################################################################
...
ggsave(glue::glue("{resub.plots}FS18_Allele_Swap_Brood_Length.pdf"), 
       height = 6, 
       width = 10)
...
ggsave(glue::glue("{resub.plots}FS19_Rescue_PC1_Brood_Size_Correlation.pdf"), 
       height = 10, 
       width = 16)
...
ggsave(glue::glue("{resub.plots}FS20_Rescue_Trait-correlation_loadings.pdf"), 
       height = 12, 
       width = 20)
...
write.table(fet_df, 
            file = glue::glue("{resub.data}Supplemental_DataXX_Human_Arsenic_Exact_test.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
            row.names = F )
