...
ggsave(paste0(plot.dir,"GWA_",trait_of_interest,"_boxplot_colored_by_ben1.png"), 
       height = 4, 
       width = 12)
...
write.table(regress_ben1, 
            file = paste0(data.dir,"GWAS_",trait_of_interest,"_with_ben1_covariate.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
            row.names = F)
...
# ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ # # ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ #
# Read data
raw <- read_data(paste0(raw_data.dir,"20180312_ben1_edits/"))
raw_nocontam <- remove_contamination(raw)
summedraw <- sumplate(raw_nocontam, quantiles = TRUE)

write.table(summedraw, 
            file = paste0(data.dir,"F200Y_del_sorter_summarized.tsv"), 
            sep = "\t", 
            quote = F, 
            col.names = T, 
            row.names = F)

summedraw <- data.table::fread(paste0(data.dir,"F200Y_del_sorter_summarized.tsv"))
...
assayregressed <- easysorter::regress(summedraw, assay = FALSE)%>%
